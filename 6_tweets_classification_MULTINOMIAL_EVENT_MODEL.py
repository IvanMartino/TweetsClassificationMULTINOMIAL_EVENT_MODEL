...
using a Multinomial Event Model.
...
"""
We classify the users in FILE_USERS using the data in all_tweets.txt via a Multinomial Event algorithm.
...
def main():
    ...
    print("-----------------------------")
    print("METHOD - MULTINOMIAL EVENT MODEL")
    print("-----------------------------")


    print("Hello,\nwe will use a Multinomial Event Model \nto classify twitter users\n")
    setpath()
    ...
    if os.path.isfile(FOLDER+"/update_MEM"+str(n_features)+"_"+str(U)+".txt")==True:
        with open(FOLDER+"/update_MEM"+str(n_features)+"_"+str(U)+".txt", "r") as h:
            update=h.read()
            h.close()
    #f=FOLDER+"/X_train_politic"+update+".npy"
        ...
        with open(FOLDER+"/X_train_politic"+today_string+"_MEM"+str(D)+"_"+str(U)+".npy",'wb') as f:
            pickle.dump(X_train,f)


        file=FOLDER+"/X_train_politic"+today_string+"_MEM"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(X_train,f)

        file=FOLDER+"/Y_train_politic"+today_string+"_MEM"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(Y_train,f)

        file=FOLDER+"/X_dev_politic"+today_string+"_MEM"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(X_dev,f)

        file=FOLDER+"/Y_dev_politic"+today_string+"_MEM"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(Y_dev,f)

        file=FOLDER+"/X_test_politic"+today_string+"_MEM"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(X_test,f)

        file=FOLDER+"/Y_test_politic"+today_string+"_MEM"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(Y_test,f)

        with open(FOLDER+"/update_MEM"+str(D)+"_"+str(U)+".txt", "w") as h:
            h.write(today_string+"_MEM"+str(D)+"_"+str(U))
            h.close()
    ...
    model_MEM = MultinomialNB()
    model_MEM.fit(X_train, Y_train)

    acc=model_MEM.score(X_train, Y_train)
    acc_dev=model_MEM.score(X_dev, Y_dev)
    print("Accuracy on the training set", acc)
    print("Accuracy on the development set", acc_dev)


    df_score, df_fp, df_pre=compute_accuracies(model_MEM,1,screen_names, X_train[:,0:n_features], X_dev[:,0:n_features], Y_train, Y_dev)
